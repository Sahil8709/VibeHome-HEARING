...
const Dropdown = ({ items, isOpen }) => (
  <div className={`absolute top-full left-0 mt-2 w-60 bg-white rounded-xl shadow-xl border border-gray-100 z-50 transition-all duration-200 ${isOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
    <div className="py-2">
      {items.map((item) => (
...
const Navbar = () => {
  ...
  return (
    <nav ref={navRef} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-3' : 'bg-white/95 backdrop-blur-sm py-4'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">

          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 group">
            <img src="/logo.png" alt="VibeHome Hearing & Speech" className="h-10 w-auto"
              onError={(e) => { e.target.style.display = 'none' }} />
            <div className="leading-tight">
              <span className="block text-blue-700 font-bold text-lg leading-none">VibeHome</span>
              <span className="block text-teal-600 text-xs font-semibold tracking-wide">HEARING & SPEECH</span>
            </div>
          </Link>
            ...
            {navLinks.map((link) => (
              <div key={link.label} className="relative"
                ...
                {link.path ? (
                  ...
                  <button className="px-3 py-2 text-sm font-semibold text-gray-700 hover:text-blue-700 rounded-lg hover:bg-blue-50 transition-all duration-150 flex items-center gap-1">
                    {link.label}
                ...
                {link.dropdown && <Dropdown items={link.dropdown} isOpen={openDropdown === link.label} />}
              </div>
            ))}
          </div>
